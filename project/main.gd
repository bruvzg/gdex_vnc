...
func handle_mouse(event):
	...
	mouse_pos2D.x = mouse_pos2D.x * vnc_texture.get_desktop_size().x
	mouse_pos2D.y = mouse_pos2D.y * vnc_texture.get_desktop_size().y
	...
	vnc_texture.send_mouse_event(event.position, event.button_mask)
...
func _conn():
	print("connected")
	vnc_texture.update_texture(1)
	node_quad.mesh.material.albedo_texture = vnc_texture
	node_quad.mesh.material.albedo_color = Color(1, 1, 1)


func _dconn():
	node_quad.mesh.material.albedo_color = Color(0, 0, 0)
	node_quad.mesh.material.albedo_texture = null
	print("disconnected")
...
func _process(delta):
	if vnc_texture and vnc_texture.status() == GDEXVNC_Texture.VNC_CONNECTED:
